...
Vec3f SSLib::SunDirection(float elevation)
{
    float inclination = DegreesToRadians(90.0f - elevation);
    float azimuth = DegreesToRadians(90.0f);
    return { std::sin(inclination) * std::cos(azimuth), std::sin(inclination) * std::sin(azimuth),
             std::cos(inclination) };
}
...
Vec3f SSLib::SunDirection(float timeOfDay, float timeZone, int julianDay, float latitude, float longitude)
{
    ...
    float inclination = DegreesToRadians(90.0f - 23.0f);
    float azimuth = DegreesToRadians(90.0f);
    return { std::sin(inclination) * std::cos(azimuth), std::sin(inclination) * std::sin(azimuth),
             std::cos(inclination) };
#endif
