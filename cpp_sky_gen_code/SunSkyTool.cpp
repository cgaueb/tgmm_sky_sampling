...
namespace
{
    float luma(Vec3f val) {
        const float YWeight[3] = { 0.212671f, 0.715160f, 0.072169f };
        return YWeight[0] * val[0] + YWeight[1] * val[1] + YWeight[2] * val[2];
    }

    void SkyToPanoramic(const cSunSky& sunSky, int height, uint8_t* data, float* fdata, int stride, const cMapInfo& mi)
    {
        float invGamma = 1.0;
        float* odata = fdata;
        ...
        for (int i = 0; i < height; i++) {
            ...
            float theta = float(height - i - 1 + 0.5) / height * vl_pi;
            if (theta > vl_pi / 2.) {
                continue;
            }
            for (int j = 0; j < width; j++) {
                float phi = float(j + 0.5) / width * 2. * vl_pi;

                Vec3f dir(std::cos(phi) * std::sin(theta),
                    std::sin(phi) * std::sin(theta), std::cos(theta));

                Vec3f c = sunSky.SkyRGB(dir);
                fdata[j] = luma(c) * std::sin(theta);
                c = mi.toneMap(c, mi.weight);
                c = pow(c, invGamma);
        ...
        float max_luma = 0.0f;
        float min_luma = 1000000.0f;
        for (int i = 0; i < height; i++) {
            uint32_t* row = (uint32_t*)data;
            float theta = float(height - i - 1 + 0.5) / height * vl_pi;
            for (int j = 0; j < width; j++) {
                max_luma = std::max(max_luma, odata[i * width + j]);
                min_luma = std::min(min_luma, odata[i * width + j]);
            }
        }
    ...
    void SkyToPanoramic(const cSunSky& sunSky, int height, Vec3f* data, const cMapInfo& mi)
    {
        ...
        for (int i = 0; i < height; i++) {
            ...
            float theta = float(i + 0.5) / height * vl_pi;
            if (theta > vl_pi / 2.) {
                continue;
            }
            for (int j = 0; j < width; j++) {
...
int main(int argc, const char* argv[])
{
    ...
    int turbidity_start = 2;
    int turbidity_stop = 6; /* Default to Preetham  */
    int elevation_start = 2;
    int elevation_stop = 90;
    int elevation_step = 3;
    const char* name = "preetham"; /* Default to Preetham  */
    ...
    if (panoramic)
    {
        for (int turbidity = turbidity_start; turbidity <= turbidity_stop; turbidity++) {
            for (int elevation = elevation_start; elevation <= elevation_stop; elevation += elevation_step) {

                Vec3f sunDir = SunDirection(elevation);
                ...
                uint32_t* image = new uint32_t[height * width];
                Vec3f* imageHDR = new Vec3f[height * width];
                float* imagef = new float[height * width];
                memset(image, 0, height * width * sizeof(*image));
                memset(imagef, 0, height * width * sizeof(*imagef));
                memset(imageHDR, 0, height * width * sizeof(*imageHDR));

                /* Build PDF and tone mapped sky map */
                SkyToPanoramic(sunSky, height, (uint8_t*)image, imagef, 0, mi);

                /* Build radiance map */
                SkyToPanoramic(sunSky, height, &imageHDR[0], mi);

                snprintf(fileName, 32, "%s_%d_%d.exr", name, elevation, turbidity);

                const char* err;
                const auto ret = SaveEXR((const float*)imageHDR, width, height, 3, /*fp16*/0, fileName, &err);
                ...
                height = height / 2;
                FIBITMAP* bitmap = FreeImage_AllocateT(FIT_FLOAT, (int)width, (int)height, 32, 0, 0, 0);
                if (0 == bitmap) {
                    printf("Failed to save file %s\n", fileName);
                    delete[] imageHDR;
                    delete[] imagef;
                ...
                memcpy(bits, imagef, width * height * sizeof(float));

                snprintf(fileName, 32, "%s_%d_%d.tiff", name, elevation, turbidity);
